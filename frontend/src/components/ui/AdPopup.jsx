...
const INITIAL_DELAY = 3000;           // 3s after page load before first show
...
export default function AdPopup() {
  ...
  const indexRef = useRef(-1); // -1 so first open always lands on index 0

  const open = useCallback(() => {
    if (sessionStorage.getItem('ad_dismissed')) return;
    const next = indexRef.current === -1 ? 0 : (indexRef.current + 1) % ADS.length;
