...
const REPORT_KEY = 'ap_report_data';
...
export default function Report() {
  ...
  // Save to sessionStorage when we have fresh data, restore if we don't
  useEffect(() => {
    if (state?.formData) {
      sessionStorage.setItem(REPORT_KEY, JSON.stringify({ formData: state.formData, associateId: state.associateId }));
    }
  }, [state]);

  const saved    = (() => { try { return JSON.parse(sessionStorage.getItem(REPORT_KEY) || '{}'); } catch { return {}; } })();
  const formData = state?.formData || saved?.formData || {};
  const assocId  = state?.associateId || saved?.associateId;

  const { personal, professional, documents, bank, referral } = formData;
          ...
          <div className="text-right">
            ...
            {assocId && (
              <>
                ...
                <p className="text-sm font-bold text-[#e85d26]">{assocId}</p>
              </>
            )}
          </div>
        </div>
        ...
        {personal?.fullName ? (
          <>
            ...
            <Section title="Document Details">
              ...
              <Row label="Aadhaar Document" value={documents?.aadhaarFile?.name || (documents?.aadhaarFile ? 'Uploaded' : '—')} />
              <Row label="PAN Number"       value={documents?.panNumber} />
              <Row label="PAN Document"     value={documents?.panFile?.name || (documents?.panFile ? 'Uploaded' : '—')} />
            </Section>
            ...
            {(referral?.associateRefNo || referral?.associateName) && (
          ...
          <div className="text-center py-12">
            ...
            <button onClick={() => navigate('/register')} className="btn-primary inline-flex">Start Registration</button>
          </div>
        )}
      </div>
    </div>
