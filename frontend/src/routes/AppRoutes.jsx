...
const isAwaitingApproval = (user) =>
  user?.isSelfRegistered && !user?.isActive;
...
const PrivateRoute = ({ children, roles }) => {
  ...
  if (isAwaitingApproval(user)) return <Navigate to="/awaiting-approval" replace />;
  if (roles) {
...
// Register is accessible to admins AND self-registered users (for re-submission after rejection)
const RegisterRoute = () => {
  const { user, loading } = useAuth();
  if (loading) return null;
  if (!user) return <Navigate to="/login" replace />;
  ...
  if (roles.includes('head_admin') || roles.includes('admin') || user.isSelfRegistered) {
    return <RegisterAssociate />;
  }
  return <Navigate to="/admin/dashboard" replace />;
};

// Awaiting approval page — only for pending self-registered users
const ApprovalRoute = () => {
  const { user, loading } = useAuth();
  if (loading) return null;
  if (!user) return <Navigate to="/login" replace />;
  if (!isAwaitingApproval(user)) return <Navigate to="/admin/dashboard" replace />;
